...
bot = AI(0.01, 0.05, [8, 4, 2], 1, states, 0.2, [10, 2, 1])  # alpha, gamma, theta, delta, states
...
while True:
    ...
    while not done:
        ...
        action[0] = np.random.normal(action[0], bot.alpha * 10 * np.sqrt(action[0]))
        if action[0] < 0:
            action[0] = 0
        if action[0] > 1.5:
            action[0] = 1.5
        observation, reward, done = env.step(action)
        if len(bot.temporary) > bot.quantity:
            bot.temporary.pop(0)
        bot.temporary.append([observation + action, reward])
    theta = bot.Deep_Update(bot.temporary)
    print(f"{action} {prediction} {reward} {bot.states[1]} {bot.alpha}")
