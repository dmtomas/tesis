...
bot = AI(0.01, 0.05, [8, 2], 1, states, 0.2, [10, 1])  # alpha, gamma, theta, delta, states
bot.LoadAI(saved_dqn)
bot.temporary = []
while True:
    observation, done = env.reset()
    bot.rewards = []
    bot.epsilon = np.abs(bot.states[0] - bot.states[1])/5
    while not done:
        action = bot.Policy(observation)
        observation, reward, done = env.step(action)
        bot.rewards.append(reward)
        if len(bot.temporary) > 50:
            bot.temporary.pop(0)
        bot.temporary.append([observation + action, reward])
    theta = bot.Deep_Update(bot.temporary)
    #print(bot.neural_policy.prev_grad)
