...
def Optimo(results):
    a = sp.dual_annealing(all_together_now, bounds=bounds, maxiter=itera)
    print("chau")
    results.append([a.fun, a.x])
...
if __name__ == '__main__':
        ...
        for j in range(0, times):
            ...
            results = []
            Optimo(results)
            #p1 = mp.Process(target=Optimo, args=(results,))
            #p2 = mp.Process(target=Optimo, args=(results,))
            #p1.start()
            #p2.start()
            #p1.join()
            #p2.join()
